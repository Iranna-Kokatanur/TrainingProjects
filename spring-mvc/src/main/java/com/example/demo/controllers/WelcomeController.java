...
// 
   ...
   public String submitForm(@Valid @ModelAttribute("command") Student student,BindingResult result) {
     ...
     if(result.hasErrors()) {
    	 ...
    	 template.postForObject("http://localhost:1010/api/v1/students", requestBody, Student.class);
     }
    return nextStep;
   }
   ...
   Student[] resp =template.getForObject("http://localhost:1010/api/v1/students",
   Student[].class);
